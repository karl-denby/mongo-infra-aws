  ...
  content = templatefile("${path.module}/template/om-vars.tpl",
    {
      ...
      amd64_backing_private = [for vps in aws_instance.amd64_backing_appdb: vps.private_dns]
      amd64_backing_public = [for vps in aws_instance.amd64_backing_appdb: vps.public_dns]
    }
  )
